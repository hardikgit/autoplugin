  ...
  *
  ...
  class pagination
  {
    ...
    function generate($array, $perPage = 10)
    {
      ...
      if (!empty($_GET['pages'])) {
        $this->page = $_GET['pages']; // using the get method
      } else {
      ...
      if (($this->pages) > 1)
      {
        ...
        if ($this->page != 1) {
          if ($this->showFirstAndLast) {
            $plinks[] = ' <a href="?pages=1'.$queryURL.'">«« First </a> ';
          }
          $plinks[] = ' <a href="?pages='.($this->page - 1).$queryURL.'">« Prev</a> ';
        }
        ...
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            ...
            $links[] = ' <a href="?pages='.$j.$queryURL.'">'.$j.'</a> '; // add the link to the array
          }
        }
   
        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a href="?pages='.($this->page + 1).$queryURL.'"> Next » </a> ';
          if ($this->showFirstAndLast) {
            $slinks[] = ' <a href="?pages='.($this->pages).$queryURL.'"> Last »» </a> ';
          }
        }
